...
public abstract class Environment extends EndpointManager {
	...
	private AsyncTask<Void, Void, Wrapper<ApiStatus, Exception>> mGetVersionTask = new AsyncTask<Void, Void, Wrapper<ApiStatus, Exception>>() {
		@Override
		protected Wrapper<ApiStatus, Exception> doInBackground(Void... params) {
			return GetStatusHttpRequest.makeRequest(Environment.this).execute();
		}

		@Override
		protected void onPostExecute(Wrapper<ApiStatus, Exception> result) {
			try {
